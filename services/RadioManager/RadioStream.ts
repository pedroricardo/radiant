...
const DEMO_AUDIO_FILE_PATH = fileURLToPath(new URL("../../Fujii Kaze - Matsuri.m4a", import.meta.url))
...
const startRadio = Effect.fn("RadioStream.startRadio")(function* (radioId: Radio.RadioId) {
	...
	const playoutManagerFiber = yield* Effect.gen(function* () {
		const audioFileSource = yield* AudioSource.fromAudioFile(DEMO_AUDIO_FILE_PATH)
		yield* multiplexer.setCluster([{ id: "matsuri", source: audioFileSource }], {
			crossfadeDuration: 0,
		})

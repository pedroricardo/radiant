...
import { MediaLibrary, MediaNode, Playout, type Radio } from "@radiant/client"
import type { ScheduleOneOffBlock, ScheduleWeeklyBlock } from "@radiant/client/lib/Playout"
import { eq } from "drizzle-orm"
import { Array as Arr, Data, DateTime, Duration, Effect, Match, Option, Order, Schema, Stream, pipe } from "effect"
...
const blockOccurrenceOrder = Order.mapInput(dateTimeOrder, (occurrence: BlockOccurrence) => occurrence.startsAt)
const nextPlanOrder = Order.mapInput(dateTimeOrder, (nextPlan: PlayoutState.NextPlan) => nextPlan.at)
...
const resolveCurrentPlanFromOccurrence = Effect.fn("PlayoutResolver.resolveCurrentPlanFromOccurrence")(
	function* (occurrence: BlockOccurrence, radioId: Radio.RadioId, nowUtc: DateTime.Utc) {
		...
		if (block.block.targetType !== "audio_file" || block.block.mediaNodeId == null) {
			return Option.none<PlayoutState.CurrentPlan>()
		}
		const mediaNode = yield* resolveMediaNode(radioId, block.block.mediaNodeId)
		...
		return Option.some(PlayoutState.CurrentPlan.AudioFile({
...
const resolveFuturePlanFromOccurrence = Effect.fn("PlayoutResolver.resolveFuturePlanFromOccurrence")(
...
export const fetchBlocks = Effect.fn("PlayoutResolver.fetchBlocks")(function* (radioId: Radio.RadioId) {
	const db = yield* Drizzle
	const { allOneOffBlocks, allWeeklyBlocks } = yield* Effect.all(
		{
			allOneOffBlocks: Effect.tryPromise({
				try: () =>
					db.select().from(scheduleOneOffBlocks).where(eq(scheduleOneOffBlocks.radioId, radioId)),
						catch: (cause) =>
				...
				}).pipe(Effect.flatMap(Schema.decode(Schema.Array(Playout.ScheduleOneOffBlock)))),
			allWeeklyBlocks: Effect.tryPromise({
				...
				}).pipe(Effect.flatMap(Schema.decode(Schema.Array(Playout.ScheduleWeeklyBlock)))),
		},
		{ concurrency: 2 },
	)
...
export const resolveTimelineSnapshot = Effect.fn("PlayoutResolver.resolveTimelineSnapshot")(function* (
	radioId: Radio.RadioId,
) {
	...
	const current = Option.getOrElse(
		currentPlanOption,
		() => PlayoutState.CurrentPlan.Silence(),
	)
	...
	yield* Effect.logDebug("playout.resolver.snapshot").pipe(
		Effect.annotateLogs({
			...
			currentBlockId: current._tag === "AudioFile" || current._tag === "Playlist" ? current.blockId : null,
			nextAt: pipe(nextOption, Option.map((next) => DateTime.toEpochMillis(next.at)), Option.getOrNull),
			nextPlan: pipe(nextOption, Option.map((next) => next.plan._tag), Option.getOrNull),
			nextBlockId:
				pipe(
					nextOption,
					Option.filterMap((next) =>
						next.plan._tag !== "AudioFile" && next.plan._tag !== "Playlist" ? Option.none() : Option.some(next.plan.blockId),
					),
					Option.getOrNull,
				),
...
export const applyCurrentPlanToMultiplexer = Effect.fn("PlayoutResolver.applyCurrentPlanToMultiplexer")(
	function* (
		radioId: Radio.RadioId,

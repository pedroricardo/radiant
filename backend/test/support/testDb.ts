...
export class WaitForDbReadyError extends Data.TaggedError("TestDbWaitForReadyError")<{
	cause: unknown
}> {}
export class MigrationSqlError extends Data.TaggedError("TestDbMigrationSqlError")<{
	cause: unknown
	file?: string
}> {}
...
const runMigrationsDir = (sql: Sql) =>
	Effect.gen(function* () {
		...
		const sorted = files.filter((f) => f.endsWith(".sql")).sort((a, b) => a.localeCompare(b))
...
const sqlLayer = Layer.scoped(
	...
	Effect.acquireRelease(
		Effect.gen(function* () {
			...
			const container = yield* Effect.tryPromise({
				try: () =>
					new GenericContainer("postgres")
						...
						.withWaitStrategy(new NoWaitingStrategy())
						.start(),
			...
			yield* Effect.tryPromise(() =>
				bootstrapDBConnection`CREATE DATABASE ${Bun.sql(randomDatabaseName)}`.then(() =>
					bootstrapDBConnection.close({ timeout: 0 }),
				),
			)

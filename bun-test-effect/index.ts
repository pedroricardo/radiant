...
export function layer<R, E, const EnableTestServices extends boolean = true>(layer_: Layer.Layer<R, E>, options?: LayerOptions<EnableTestServices>) {
	...
	type TestEffect<E, AdditionalServices = never> = Effect.Effect<void, E, EffectiveServices | AdditionalServices>;
	type NormalEffectTester<AdditionalServices = never> = <E>(
		label: string,
		test: () => TestEffect<E, AdditionalServices>,
		options?: BunTest.TestOptions,
	) => void;
	type ConditionalEffectTester = (condition: boolean) => <E>(label: string, test: () => TestEffect<E>, options?: BunTest.TestOptions) => void;
	...
	const enableTestServices = options != null && options.testServices != null ? options.testServices : true;
	const withTestEnv = (enableTestServices ? Layer.provideMerge(layer_, TestContext.TestContext) : layer_) as LayerMaybeWithTestEnv;
	...
	const createNormalTester =
		...
		(label, test, options) => {
			rawTester(label, () => runPromise(Effect.flatMap(runtimeEffect, runtime => test().pipe(Effect.provide(runtime)))), options);
		};
	const createScopedTester =
		(rawTester: typeof bunTest.it.only): NormalEffectTester<Scope.Scope> =>
		(label, test, options) => {
			rawTester(label, () => runPromise(Effect.flatMap(runtimeEffect, runtime => test().pipe(Effect.scoped, Effect.provide(runtime)))), options);
		};
	const createConditionalTester =
		...
		condition => {
			...
			return (label, test, options) => {
				intermediateFn(label, () => runPromise(Effect.flatMap(runtimeEffect, runtime => test().pipe(Effect.provide(runtime)))), options);
			};
		};
	const createScopedBulkTester =
		...
		table => {
			...
			return (label, test, options) => {
				intermediateFn(
					label,
					(...args: any) => runPromise(Effect.flatMap(runtimeEffect, runtime => test(...args).pipe(Effect.scoped, Effect.provide(runtime)))),
					options,
				);
			};
		};
	const createBulkTester =
		...
		table => {
			...
			return (label, test, options) => {
				intermediateFn(
					label,
					(...args: any) => runPromise(Effect.flatMap(runtimeEffect, runtime => test(...args).pipe(Effect.provide(runtime)))),
					options,
				);
			};
		};
	...
	const testers: Testers<EffectiveServices> = {
		...
		layer: (layer_, options) => layer(Layer.provideMerge(layer_, withTestEnv), { ...options, memomap }),
	};
